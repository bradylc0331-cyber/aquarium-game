...
掃描進來的作品會存進這台電腦的 IndexedDB，投影分頁重新整理之後自動還原——
活動中不小心按到 F5、瀏覽器當掉、或電腦休眠後分頁被回收，孩子畫的東西不會消失。
這個驗收真的重新載入一次，確認作品找得回來、還原順序正確（FIFO 才不會擠錯人）、
超量時只留最新的 15 張，以及沒有 IndexedDB 的環境（私密視窗）畫面照常運作。

只留最新 15 張 = 畫面上同時容得下的數量。要清空重來（換一場活動）可以在
投影分頁的開發者工具執行 `__bibleDebug` 旁邊的 store，或直接清除該網站的資料。
