(function (root) {
  ...
  function getWalkableArea(width, height) {
    ...
    return {
      ...
      bottom: height * 0.93,
      obstacles: [
        // 這裡**沒有河**。河在畫面中段的遠景，角色走的前景草地上沒有水，
        // 所以不需要任何河流障礙。
        //
        // 先前這裡放了兩塊近似河流的矩形，是照著整張插畫的印象加的，沒有對著
        // 角色實際走的那條草地量。後果很嚴重：它們擋掉一大片本來能走的草地，
        // 還在彼此之間夾出一條 28px 高的死巷——實測有角色走進去之後連續
        // 294 秒出不來（四周 20px 內 0/36 個方向可走，最近的鄰居卻在 96px 外）。
        // 前六輪的規格審查都在追移動邏輯，真正的問題是這張地圖畫錯了。
  ...
  function steerCharacter(self, characters, area, dt) {
    ...
    for (const velocity of options) {
      ...
      if (progressElapsed >= PROGRESS_WINDOW_SECONDS) {
        ...
        // 不必要求更多。
        // 「走過的範圍太小」只在**沒有規劃路徑**的時候才算卡住。
        //
        // 正在照著規劃好的路走的角色，會為了貼著障礙而放慢、小心地挪——實測
        // 一位沿著牆往缺口走的角色每兩秒只挪 6.8px，低於門檻 11.2px，於是被
        // 判定卡住、規劃好的路被丟掉，然後牠又沿著牆漂回去，來回 240 秒都到
        // 不了那個缺口。有路可循時，判準應該是「離下一個轉角近了沒」，
        // 那個下面已經在算了。
        const spreadNeeded = Math.min(selfSpace.radiusX, freeTravel * MIN_SPREAD_FRACTION);
        if ((path === null && spread < spreadNeeded)
