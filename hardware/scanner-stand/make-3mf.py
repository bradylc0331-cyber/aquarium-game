...
PLATES = [
    ("plate1-gauge", "試裝規 + 試扣梳（先印這塊）", [
        ("gauge",    128, 150),
        ("clipcomb", 128, 100),
    ]),
    ("plate2-foot-knobs", "底座 + 6 顆旋鈕", [
        ("foot",  90, 125),
        ("knob", 193,  55), ("knob", 225,  55),
        ("knob", 193,  87), ("knob", 225,  87),
        ("knob", 193, 119), ("knob", 225, 119),
    ]),
    ...
    # 補印板：給已經照舊版印過一輪的人。只放「真的需要重印」的零件 ——
    # 用幾何比對（體積、外框、兩萬點佔用取樣）確認過 foot / mast / mast_short /
    # cradle / knob / gauge 的形狀完全沒變，那些不用重印。
    ("plate5-rework", "補印板：接頭 ×2（1.5 mm 修正）、旋鈕 ×2（數量補齊）、試扣梳 ×1", [
        ("clipcomb", 128,  40),
        ("plug",     100, 100), ("plug", 156, 100),
        ("knob",     100, 150), ("knob", 156, 150),
    ]),
]
